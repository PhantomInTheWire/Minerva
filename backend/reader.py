...
def neighbor_select(state: OverallState) -> OverallState:
    ...
    if chosen_action.get("function_name") == "read_neighbor_node" and chosen_action.get("arguments") and len(chosen_action.get("arguments")) > 0:
        ...
        if state.get("neighbor_check_queue") and len(state.get("neighbor_check_queue")[0].get("possible_candidates", [])) > 0:
...
def answer_reasoning(state: OverallState) -> OutputState:
    ...
    if not state.get("notebook"):
        return {
            "answer": "Unable to provide an answer as no relevant information was found.",
            "analysis": "No relevant chunks of information were found to analyze.",
    ...
    try:
        ...
        return {
            ...
            "analysis": "An error occurred while analyzing the available information.",
            "previous_actions": ["answer_reasoning"]
        }
